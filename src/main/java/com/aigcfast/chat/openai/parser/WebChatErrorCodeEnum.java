 ...
 */
@AllArgsConstructor
public enum WebChatErrorCodeEnum {
     ...
     * code 作为 key，封装为 Map
     */
    public static final Map<String,WebChatErrorCodeEnum> CODE_MAP = Stream
            .of(WebChatErrorCodeEnum.values())
            .filter(e -> StrUtil.isNotBlank(e.getCode()))
            .collect(Collectors.toMap(WebChatErrorCodeEnum ::getCode, Function.identity()));

    /**
     * detail 作为 key，封装为 Map
     */
    public static final Map<String,WebChatErrorCodeEnum> DETAIL_MAP = Stream
            .of(WebChatErrorCodeEnum.values())
            .filter(e -> StrUtil.isNotBlank(e.getDetail()))
            .collect(Collectors.toMap(WebChatErrorCodeEnum ::getDetail, Function.identity()));
}
